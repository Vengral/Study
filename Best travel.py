...
def choose_best_sum(t, k, ls):
    ...
    possible_combinations = list(combinations(ls, k))
    
 
    valid_combinations = [comb for comb in possible_combinations if sum(comb) <= t]
    
    print(valid_combinations)
    # your code
    result = max(valid_combinations, default=None, key=sum)
    return sum(result) if result is not None else None
